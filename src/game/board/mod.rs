...
impl Board {
    pub(crate) fn init() -> Self {
        ...
        // Pawns
        for j in 0..COLUMNS {
            board_builder.add(PieceKind::Pawn(Pawn::new((1usize, j).into(), Color::Black)));
        }
        for j in 0..COLUMNS {
            board_builder.add(PieceKind::Pawn(Pawn::new((6usize, j).into(), Color::White)));
        }
    ...
    pub(super) fn piece_mut(&mut self, position: Position, color: Color) -> Option<&mut PieceKind> {
        self.square_mut(position)?.piece_mut(color)
    }

    pub(super) fn piece_unset(&mut self, position: Position) -> Option<PieceKind> {
        self
            .square_mut(position)
    ...
    pub(super) fn set_piece(&mut self, position: Position, piece: PieceKind) {
        self
            .square_mut(position)
            .expect("The square in position {position:?} should exist")
            .set_piece(piece);
    ...
    pub fn king_side_rook_mut(&mut self, color: Color) -> Option<&mut PieceKind> {
        if let PieceKind::King(king) = self.king_mut(color)? {
            let rook_position: Position = king.king_side_castling_rook_position();
            return self.piece_mut(rook_position, color);
        }

        None
    }

    pub fn queen_side_rook_mut(&mut self, color: Color) -> Option<&mut PieceKind> {
        if let PieceKind::King(king) = self.king_mut(color)? {
    ...
    pub fn king(&self, color: Color) -> Option<&PieceKind> {
        self.piece(self.king_position(color)?, color)
    }

    pub fn king_mut(&mut self, color: Color) -> Option<&mut PieceKind> {
        self.piece_mut(self.king_position(color)?, color)
    }
    ...
    pub fn checked(&self, color: Color) -> bool {
        if let Some(king_position) = self.king_position(color) {
            self
...
mod tests {
    ...
    fn test_square_mut(
        ...
        position: Position,
        #[case]
        mut expected: Option<Square>
        ) {
        let mut board: Board = BoardBuilder::new().build();
        let square: Option<&mut Square> = board.square_mut(position);

        assert_eq!(expected.as_mut(), square);
    }

    #[rstest]
    #[case((1isize, 3isize).into(), Some(PieceKind::Pawn(Pawn::new((1isize, 3isize).into(), Color::Black))))]
    #[case((2isize, 3isize).into(), None)]
    fn test_piece(
        #[case]
        position: Position,
        #[case]
        ...
        let piece: Option<&PieceKind> = board.piece(position, Color::Black);

        assert_eq!(expected.as_ref(), piece);
    }
    ...
    fn test_set_attacking() {
        let mut board: Board = BoardBuilder::new()
            ...
            .with(PieceKind::Bishop(Bishop::new((6isize, 5isize).into(), Color::Black)))
            .with(PieceKind::Pawn(Pawn::new((1isize, 1isize).into(), Color::White)))
            .build();
    ...
    fn test_make_move_attack() {
        ...
        let expected: Board = BoardBuilder::new()
            .with(PieceKind::Bishop(Bishop::new((1isize, 2isize).into(), Color::Black)))
            .build();
    ...
    fn test_make_move_pawn_move() {
        ...
        let mut board: Board = BoardBuilder::new()
            ...
            .build();
        let expected: Board = BoardBuilder::new()
            .with(PieceKind::Pawn(Pawn::new((2isize, 2isize).into(), Color::Black).with_has_moved()))
            .build();
    ...
    fn test_make_move_en_passant() {
        ...
        let mut board: Board = BoardBuilder::new()
            ...
            .build();
        let expected: Board = BoardBuilder::new()
            .with(PieceKind::Pawn(Pawn::new((5isize, 3isize).into(), Color::Black)))
            .build();
    ...
    fn test_make_move_ignore_en_passant() {
        ...
        let expected: Board = BoardBuilder::new()
            ...
            .with(PieceKind::Pawn(Pawn::new((4isize, 3isize).into(), Color::White).with_has_moved()))
            .build();
    ...
    fn test_make_move_queen_side_castle() {
        ...
        let mut board: Board = BoardBuilder::new()
            ...
            .build();
        let expected: Board = BoardBuilder::new()
            .with(PieceKind::King(King::new((0isize, 2isize).into(), Color::Black).with_has_moved()))
            .with(PieceKind::Rook(Rook::new((0isize, 3isize).into(), Color::Black).with_has_moved()))
            .build();
    ...
    fn test_make_move_king_side_castle() {
        ...
        let mut board: Board = BoardBuilder::new()
            ...
            .build();
        let expected: Board = BoardBuilder::new()
            .with(PieceKind::King(King::new((0isize, 6isize).into(), Color::Black).with_has_moved()))
            .with(PieceKind::Rook(Rook::new((0isize, 5isize).into(), Color::Black).with_has_moved()))
            .build();
    ...
    fn test_checked() {
        let mut board: Board = BoardBuilder::new()
            .with(PieceKind::King(King::new((0isize, 4isize).into(), Color::Black)))
            .with(PieceKind::Rook(Rook::new((5isize, 4isize).into(), Color::White)))
            .build();
    ...
    fn test_not_checked() {
        let mut board: Board = BoardBuilder::new()
            .with(PieceKind::King(King::new((0isize, 4isize).into(), Color::Black)))
            .with(PieceKind::Rook(Rook::new((5isize, 5isize).into(), Color::White)))
            .build();
        board.set_possible_moves(Color::White);
